...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            SITEMAP,
            SitemapFilter(date_range=DateRange(start=datetime(2025, 3, 15), end=datetime(2025, 3, 20))),
            SITEMAP_LOC[8:],
        ),
        ...
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(contains=r"(page1)|(section1)"),
            [SITEMAP_WITH_ONLY_URLS_LOC[i] for i in [1, 5, 6, 7]],
        ),
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(contains=r"(page1)|(section1)", skip=1, take=2),
            [SITEMAP_WITH_ONLY_URLS_LOC[i] for i in [5, 6]],
        ),
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(contains=r"^((?!section1).)*$"),
            [SITEMAP_WITH_ONLY_URLS_LOC[i] for i in [0, 1, 2, 3, 4, 7, 8]],
        ),
        ...
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(excludes="/page"),
            [SITEMAP_WITH_ONLY_URLS_LOC[i] for i in [0, 5, 6, 7, 8]],
        ),
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(excludes=r"(page1)|(section?\d)"),
            [SITEMAP_WITH_ONLY_URLS_LOC[i] for i in [0, 2, 3, 4]],
        ),
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(excludes="subpage"),
            [SITEMAP_WITH_ONLY_URLS_LOC[i] for i in [0, 1, 2, 3, 4]],
        ),
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(change_frequency=ChangeFrequency.DAILY),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <changefreq> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(change_frequency="daily"),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <changefreq> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=DateRange(start=datetime(2025, 3, 15), end=datetime(2025, 3, 20))),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=Between(datetime(2025, 1, 20), datetime(2025, 2, 20))),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=Today()),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=Yesterday()),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=DaysAgo(2)),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=DaysAgo((datetime.today() - datetime(2025, 3, 15)).days)),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=LaterThan(datetime(2025, 3, 10))),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=LaterThanAndIncluding(datetime(2025, 3, 10))),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=EarlierThan(datetime(2025, 1, 20))),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        (
            SITEMAP_WITH_ONLY_URLS,
            SitemapFilter(date_range=EarlierThanAndIncluding(datetime(2025, 1, 20))),
            SITEMAP_WITH_ONLY_URLS_LOC,
        ),  # Bypassed due to no <lastmod> element.
        ...
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(contains=r"(page1)|(section1)"),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [1, 5, 6, 7]],
        ),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(contains=r"(page1)|(section1)", skip=1, take=2),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [5, 6]],
        ),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(contains=r"^((?!section1).)*$"),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [0, 1, 2, 3, 4, 7, 8]],
        ),
        ...
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(excludes=r"(page1)|(section?\d)"),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [0, 2, 3, 4]],
        ),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(excludes="subpage"),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [0, 1, 2, 3, 4]],
        ),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(change_frequency=ChangeFrequency.ALWAYS),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [0, 1, 2, 4, 5, 7, 8]],
        ),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(change_frequency="always"),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [0, 1, 2, 4, 5, 7, 8]],
        ),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(date_range=DateRange(start=datetime(2025, 3, 15), end=datetime(2025, 3, 20))),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [1, 4, 5, 7, 8]],
        ),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(date_range=Between(datetime(2025, 1, 20), datetime(2025, 2, 20))),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [1, 3, 4, 5, 7]],
        ),
        (SITEMAP_INCONSISTENT, SitemapFilter(date_range=Today()), [SITEMAP_INCONSISTENT_LOC[i] for i in [1, 4, 5, 7]]),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(date_range=Yesterday()),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [1, 4, 5, 7]],
        ),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(date_range=DaysAgo(2)),
        ...
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(date_range=EarlierThan(datetime(2025, 1, 20))),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [0, 1, 4, 5, 7]],
        ),
        (
            SITEMAP_INCONSISTENT,
            SitemapFilter(date_range=EarlierThanAndIncluding(datetime(2025, 1, 20))),
            [SITEMAP_INCONSISTENT_LOC[i] for i in [0, 1, 2, 4, 5, 7]],
        ),
    ],
)
